...
export default function OrbitCanvas({ orionFraction, telemetry }) {
  ...
  const draw = useCallback(() => {
    ...
    ctx.save();
    const rng = { v: 42 };
    const lcg = () => { rng.v = (rng.v * 1664525 + 1013904223) & 0xFFFFFFFF; return (rng.v >>> 0) / 0xFFFFFFFF; };
    for (let i = 0; i < 180; i++) {
      const sx = lcg() * W, sy = lcg() * H;
      if (Math.hypot(sx - cx, sy - cy) < displayR * 0.95) continue;
      const alpha = 0.1 + lcg() * 0.5 + 0.15 * Math.sin(frame * 0.02 + lcg() * 6.28);
      ctx.fillStyle = `rgba(200,220,255,${alpha.toFixed(2)})`;
      ctx.beginPath(); ctx.arc(sx, sy, lcg() * 1.4 + 0.3, 0, Math.PI * 2); ctx.fill();
    }
    ctx.restore();
    ...
    [100000, 200000, 300000, 400000].forEach(km => {
      const r = km * scale;
      ctx.strokeStyle = 'rgba(0,120,200,0.12)'; ctx.lineWidth = 0.5;
    ...
    for (let a = 0; a < 360; a += 45) {
      ...
      ctx.strokeStyle = 'rgba(0,100,180,0.07)'; ctx.lineWidth = 0.5;
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(cx + Math.cos(rad) * displayR * 1.1, cy - Math.sin(rad) * displayR * 1.1);
      ctx.stroke();
    }
    ctx.restore();
    ...
    if (split > 1) {
      ctx.save();
      const grad = ctx.createLinearGradient(spline[0].x, spline[0].y, spline[split].x, spline[split].y);
      grad.addColorStop(0, 'rgba(0,100,200,0.15)');
      grad.addColorStop(1, 'rgba(0,180,255,0.5)');
      ctx.strokeStyle = grad; ctx.lineWidth = 1.5 / zoom; ctx.setLineDash([]);
    ...
    if (split < N - 1) {
      ctx.save();
      ctx.strokeStyle = 'rgba(0,150,255,0.18)'; ctx.lineWidth = 1 / zoom;
      ctx.setLineDash([4 / zoom, 6 / zoom]);
      ctx.beginPath(); ctx.moveTo(spline[split].x, spline[split].y);
    ...
    moonGlow.addColorStop(0, 'rgba(180,180,160,0.1)'); moonGlow.addColorStop(1, 'transparent');
    ctx.fillStyle = moonGlow; ctx.beginPath(); ctx.arc(mx, my, moonPx * 3, 0, Math.PI * 2); ctx.fill();
    const moonBody = ctx.createRadialGradient(mx - moonPx * 0.3, my - moonPx * 0.3, 0, mx, my, moonPx);
    moonBody.addColorStop(0, '#D0C8BC'); moonBody.addColorStop(0.6, '#8A8070'); moonBody.addColorStop(1, '#4A4440');
    ctx.fillStyle = moonBody; ctx.beginPath(); ctx.arc(mx, my, moonPx, 0, Math.PI * 2); ctx.fill();
    ...
    for (let r = 3; r >= 0; r--) {
      const atmoGlow = ctx.createRadialGradient(cx, cy, earthPx, cx, cy, earthPx + (r + 1) * 12);
      atmoGlow.addColorStop(0, `rgba(20,100,255,${0.10 - r * 0.02})`);
      atmoGlow.addColorStop(1, 'transparent');
      ctx.fillStyle = atmoGlow; ctx.beginPath(); ctx.arc(cx, cy, earthPx + (r + 1) * 12, 0, Math.PI * 2); ctx.fill();
    }
    const earthBody = ctx.createRadialGradient(cx - earthPx * 0.3, cy - earthPx * 0.3, 0, cx, cy, earthPx);
    earthBody.addColorStop(0, '#2060CC'); earthBody.addColorStop(0.5, '#0A3080'); earthBody.addColorStop(1, '#020820');
    ctx.fillStyle = earthBody; ctx.beginPath(); ctx.arc(cx, cy, earthPx, 0, Math.PI * 2); ctx.fill();
    ctx.save(); ctx.beginPath(); ctx.arc(cx, cy, earthPx, 0, Math.PI * 2); ctx.clip();
    ctx.fillStyle = 'rgba(40,110,50,0.45)';
    ctx.beginPath(); ctx.ellipse(cx - earthPx*0.3, cy - earthPx*0.2, earthPx*0.18, earthPx*0.22, -0.3, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx + earthPx*0.1, cy - earthPx*0.15, earthPx*0.10, earthPx*0.28, 0.1, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx + earthPx*0.3, cy - earthPx*0.2, earthPx*0.24, earthPx*0.18, 0.2, 0, Math.PI*2); ctx.fill();
    ctx.restore();
    ctx.strokeStyle = 'rgba(100,180,255,0.35)'; ctx.lineWidth = 2 / zoom;
    ctx.beginPath(); ctx.arc(cx, cy, earthPx, 0, Math.PI * 2); ctx.stroke();
    ...
    ctx.strokeStyle = `rgba(255,96,32,${ringAlpha})`;   ctx.lineWidth = 1 / zoom;
    ctx.beginPath(); ctx.arc(scX, scY, pulseR,  0, Math.PI * 2); ctx.stroke();
    ctx.strokeStyle = `rgba(255,96,32,${ringAlpha * 0.4})`; ctx.lineWidth = 0.5 / zoom;
    ctx.beginPath(); ctx.arc(scX, scY, pulseR2, 0, Math.PI * 2); ctx.stroke();
    const coreR = 8 / zoom;
    const scGlow = ctx.createRadialGradient(scX, scY, 0, scX, scY, coreR);
    scGlow.addColorStop(0, '#FF9944'); scGlow.addColorStop(0.4, '#FF6020'); scGlow.addColorStop(1, 'transparent');
    ctx.fillStyle = scGlow; ctx.beginPath(); ctx.arc(scX, scY, coreR, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#FF8030'; ctx.beginPath(); ctx.arc(scX, scY, 4 / zoom, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#FFD0A0'; ctx.beginPath(); ctx.arc(scX, scY, 2 / zoom, 0, Math.PI * 2); ctx.fill();
    const vDx = cur.x - prev.x, vDy = cur.y - prev.y;
    const vLen = Math.hypot(vDx, vDy) || 1;
    ctx.strokeStyle = 'rgba(255,184,0,0.5)'; ctx.lineWidth = 1 / zoom;
    ctx.setLineDash([3 / zoom, 3 / zoom]);
    ctx.beginPath();
    ctx.moveTo(scX, scY);
    ctx.lineTo(scX + (vDx / vLen) * 18 / zoom, scY + (vDy / vLen) * 18 / zoom);
    ...
    const barLabel = barKmRaw >= 1000 ? `${Math.round(barKmRaw / 1000)}k km` : `${Math.round(barKmRaw)} km`;
    const bx = 14, by = H - 18;
    ctx.strokeStyle = 'rgba(0,180,255,0.4)'; ctx.lineWidth = 0.8; ctx.setLineDash([]);
    ...
    ctx.fillStyle = 'rgba(0,160,220,0.4)'; ctx.font = '7px "JetBrains Mono", monospace';
    ctx.textAlign = 'left'; ctx.fillText(barLabel, bx, by - 6);
    ...
    ctx.roundRect(W - 100, H - 22, 90, 14, 7); ctx.fill(); ctx.stroke();
    ...
    if (zoom > 1.05) {
      ...
      ctx.roundRect(W / 2 - 30, 10, 60, 15, 7); ctx.fill(); ctx.stroke();
    ...
    if (mouse) {
      ...
      for (let i = 0; i < spline.length - 1; i++) {
        const d = ptSegDist(wMx, wMy, spline[i].x, spline[i].y, spline[i+1].x, spline[i+1].y);
      ...
      if (nearestDist < 28 / zoom && nearestIdx >= 0) {
        ...
        ctx.save();
        ctx.strokeStyle = 'rgba(0,220,255,0.65)'; ctx.lineWidth = 0.8; ctx.setLineDash([3, 3]);
        ctx.beginPath(); ctx.moveTo(hsx - 14, hsy); ctx.lineTo(hsx + 14, hsy); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(hsx, hsy - 14); ctx.lineTo(hsx, hsy + 14); ctx.stroke();
        ctx.setLineDash([]);
        ctx.strokeStyle = 'rgba(0,220,255,0.4)'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.arc(hsx, hsy, 6, 0, Math.PI * 2); ctx.stroke();

        const isPast = nFrac <= orionFraction;
        const tlW = 176, tlH = 86;
        let tlX = hsx + 14, tlY = hsy - tlH / 2;
        ...
        ctx.roundRect(tlX, tlY, tlW, tlH, 4); ctx.fill(); ctx.stroke();

        ctx.strokeStyle = 'rgba(0,220,255,0.7)'; ctx.lineWidth = 1.5;
        [[tlX, tlY, 1, 1], [tlX+tlW, tlY, -1, 1], [tlX, tlY+tlH, 1, -1], [tlX+tlW, tlY+tlH, -1, -1]].forEach(([bx, by, sx, sy]) => {
          ctx.beginPath(); ctx.moveTo(bx + sx*8, by); ctx.lineTo(bx, by); ctx.lineTo(bx, by + sy*8); ctx.stroke();
        });

        ctx.font = 'bold 8px "JetBrains Mono", monospace'; ctx.fillStyle = 'rgba(0,200,255,0.5)';
        ctx.textAlign = 'left';
        ctx.fillText(isPast ? 'TRAJECTORY PAST' : 'TRAJECTORY AHEAD', tlX+10, tlY+14);
        ctx.font = '9px "JetBrains Mono", monospace'; ctx.fillStyle = 'rgba(200,220,255,0.85)';
        ctx.fillText(`DIST   ${Math.round(interpDist).toLocaleString()} km`, tlX+10, tlY+30);
        ...
        if (!isPast) {
          ...
          if (etaMs > 0) {
            ...
            ctx.fillText(`ETA    ${etaH}h ${String(etaM).padStart(2,'0')}m`, tlX+10, tlY+58);
          }
        } else if (speedKmh) {
          ctx.fillText(`SPEED  ${Math.round(speedKmh).toLocaleString()} km/h`, tlX+10, tlY+58);
        }
        ctx.font = 'bold 7px "JetBrains Mono", monospace';
        ctx.fillStyle = isPast ? 'rgba(0,232,122,0.7)' : 'rgba(0,150,255,0.7)';
        ctx.fillText(isPast ? '● COMPLETED' : '◌ AHEAD', tlX+10, tlY+tlH-10);
        ctx.restore();
      }
    }
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      applyZoom(s.current.zoom * factor, e.clientX - rect.left, e.clientY - rect.top);
    };
  ...
  const handleMouseDown = useCallback((e) => {
    ...
    s.current.dragging = { startX: e.clientX, startY: e.clientY, panX: s.current.panX, panY: s.current.panY };
  }, []);
  ...
  const stopDrag    = useCallback(() => { s.current.dragging = null; }, []);
  const clearMouse  = useCallback(() => { s.current.mouse = null; s.current.dragging = null; }, []);
  ...
  const handleTouchStart = useCallback((e) => {
    if (e.touches.length === 2) {
      ...
      s.current.dragging = { startX: e.touches[0].clientX, startY: e.touches[0].clientY, panX: s.current.panX, panY: s.current.panY };
    }
  }, []);
  ...
  const stopTouch = useCallback(() => { s.current.dragging = null; s.current.pinch = null; }, []);
  ...
  const zoomIn = useCallback(() => {
    ...
    if (!canvas) return;
    const dpr = window.devicePixelRatio || 1;
    applyZoom(s.current.zoom * 1.5, canvas.width / dpr / 2, canvas.height / dpr / 2);
  }, [applyZoom]);
  ...
  const focusOrion = useCallback(() => {
    ...
    const idx = Math.min(Math.max(0, spline.findIndex(p => p.frac >= orionFraction)), N - 1);
    const pt  = spline[idx];
    const targetZoom = 5;
  ...
  const btnStyle = (highlight, wide) => ({
    ...
    userSelect:  'none',
    flexShrink:  0,
    boxShadow:   highlight ? '0 0 10px rgba(255,96,32,0.18)' : '0 2px 8px rgba(0,0,0,0.35)',
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', aspectRatio: '1', position: 'relative' }}>
      ...
      <div style={{
        ...
        {/* Divider */}
        <div style={{ width: 1, height: 20, background: 'rgba(0,150,255,0.2)', margin: '0 2px' }} />
      ...
      <div style={{
        ...
        color: 'rgba(70,130,190,0.38)', letterSpacing: 1.5,
